...
import { ERROR_CODE_RM_SYMBOL } from "../constants/constant";
import { ICoinPrice1AM, ICoinPrice1AMMap } from "../interfaces/coin-price-1am.interface";
import { IMarketOrderPieceEntity } from "../interfaces/market-order-piece.interface";
import { TBinanceError, TResponse, TSymbolMarketPrice, TSymbolTickerPrice } from "../types/rest-api";
...
import { TMarkPriceStream, TSymbolMarketPriceWs, TSymbolTickerPriceWs, TTickerPriceStream } from "../types/websocket";
...
export function paramsToQueryWithSignature(binance_api_secret: string, paramsObject: Record<string, unknown>): string {
...
export function binanceStreamToSymbolPrice(streamResponse: TMarkPriceStream | TTickerPriceStream) {
...
// arrray to map
export function symbolPricesToMap(symbolPrices: ICoinPrice1AM[]): ICoinPrice1AMMap {
...
export function symbolPriceTickersToMap<T extends Omit<TSymbolTickerPrice, "time">>(symbolPriceTickers: T[]) {
...
export function symbolPriceMarketsToMap(symbolPriceMarket: TSymbolMarketPrice[]) {
...
export function orderPiecesToMap(orderPieces: IMarketOrderPieceEntity[]): Record<string, IMarketOrderPieceEntity> {
...
export function symbolPnlManagingsMapFromAbleSymbols(ableSymbols: string[], initConfig: TSymbolPnlManaging) {
  let res: TSymbolPnlManagingsMap = {};
...
export function mergeTicerPriceAndMarketPriceBySymbol(tickerPrices: TSymbolTickerPrice[], marketPrices: TSymbolMarketPrice[]): TTickerAndMarkPrice[] {
  // Change array to map
...
export const ableOrderSymbolsMapToArray = (ableOrderSymbolsMap: Record<string, boolean>) => {
...
export const pnlOfSymbolFromPositionsMap = (positionsMap: TPositionsMap, symbol: string) => {
  ...
  if (positionPnl === null || positionPnl === undefined) throw new Error(`${symbol}: position object dont have unRealizedProfit property`);

  const positionPnlNumber = parseFloat(positionPnl);
  return positionPnlNumber;
};

export function removeNullUndefinedProperties(obj: Record<string, unknown>): Record<string, unknown> {
  return Object.fromEntries(Object.entries(obj).filter(([_, value]) => value !== null && value !== undefined));
}

export function maxMarketPriceKlineFromArray(marketPriceKlinesArray: TSymbolMarketPriceKline[]) {
  let maxPrice = 0;
...
export function currentMarketPriceKlineFromArray(marketPriceKlinesArray: TSymbolMarketPriceKline[]) {
...
export function numberOfBuyOrder(orderPieces: IMarketOrderPieceEntity[]): number {
...
export function isBinanceError<T>(response: TResponse<T>): response is TBinanceError {
  return (response as TBinanceError).code !== undefined;
}

// remove el in array, also mutate the array
export function removeArrayElMutate<T>(array: T[], el: T) {
...
export function isInErrorCodeToRemove(errorCode: number) {
  const errorCodes: number[] = ERROR_CODE_RM_SYMBOL;

  if (errorCodes.includes(errorCode)) return true;
  return false;
}

export function updateObject<T, K extends keyof T>(obj: T, key: K, value: T[K]): T {
  obj[key] = value;
  return obj;
}
