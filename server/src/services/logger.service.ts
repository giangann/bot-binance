...
const saveErrorAndClg = (error: any) => {
  saveError(error);
  console.log(error);
};

const saveDebug = (content: string) => {
  logger.debug(content);
};

const saveDebugAndClg = (content: string) => {
  saveDebug(content);
  console.log(content);
};
...
export default { saveError, saveDebug, saveDebugAndClg , saveErrorAndClg};
